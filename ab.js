class Jsonhostdata extends CI_Controller {
    ...
    AND web_status_id=22`
	var	qhost= this.db.query(sqlqhost);
	var	numrow= qhost.num_rows();
		var r=1;
		var bDesign=0;
		foreach(qhost->result_array() as $host){

		var sqlcus=`SELECT c.*,ol.product_id
							FROM 
							erp_website w,
							erp_orders_list ol,
							erp_orders o,
	...
	function changstaterpwebsite(){
		...
		if($data->token=='b42ec87ec4eaadc1449584c164ae7ff5'){			
			...
			if($upstat){
				...
				console.log ('{"stat":"unsuccess"}');
			}
		}else{
			console.log (`{stat:.${data}->${website_id}.}`);
		}
	}
